                    ...
                    <tr>
                        ...
                        <td style="text-align: right;">
                            <div class="btn-group">
                                ...
                                <form action="{{ route('tyro-dashboard.invitations.admin.destroy', $link->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this invitation link? {{ $link->referrals->count() > 0 ? 'This will also remove ' . $link->referrals->count() . ' referral record(s).' : '' }}');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger" title="Delete invitation link">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
